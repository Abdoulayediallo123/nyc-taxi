...
-- HOURLY PICKUP
GO
CREATE VIEW HOURLY_PICKUP_COUNT AS 
SELECT  
    DATEPART(HOUR, PICKUP_DATETIME) AS PICKUP_HOUR,
    COUNT(*) AS TOTAL_TRIPS
FROM STAGE.NYC_TAXI
...
GO
CREATE VIEW DALY_TRIP_COUNT AS
SELECT 
    CAST(PICKUP_DATETIME AS DATE) AS TRIP_DATE,
    COUNT(*) AS TOTAL_TRIP
FROM 
...
GO
CREATE VIEW WEEKLY_TRIP_COUNT AS
SELECT 
    DATEPART(WEEK, PICKUP_DATETIME) AS TRIP_BY_WEEK,
    COUNT(*) AS TOTAL_TRIP
FROM STAGE.NYC_TAXI
...
-- TRIPS BY MONTH.
GO
CREATE VIEW MONTHLY_TRIPS_COUNT AS
SELECT 
    DATEPART(MONTH, PICKUP_DATETIME) AS TRIP_BY_MONTH,
    COUNT(*) AS TOTAL_TRIP_A_MONTH
FROM STAGE.NYC_TAXI 
...
-- What are the rush hour trends (weekday vs. weekend)?
GO
CREATE VIEW RUSH_HOUR_TRENDS AS
SELECT
CAST(PICKUP_DATETIME AS DATE) AS PICKUP_DATE,
DATENAME(WEEKDAY, CAST(PICKUP_DATETIME AS DATE)) AS PICKUP_DAY,
CONCAT(DATEPART(HOUR, PICKUP_DATETIME), ' hr') AS PICKUP_TIME,
...
GO
CREATE VIEW TRIPS_DISTANCE_AND_FARES AS
SELECT  
CAST(PICKUP_DATETIME AS DATE) AS PICKUP_DATE,
DATENAME(WEEKDAY, CAST(PICKUP_DATETIME AS DATE)) AS WEEK_DAY,
CAST(PICKUP_DATETIME AS TIME) AS PICKUP_TIME,
...
GO
CREATE VIEW MONTHS_WITH_MORE_TRIPS AS
SELECT 
CAST(PICKUP_DATETIME AS DATE) AS PICKUP_DATE,
DATENAME(MONTH, PICKUP_DATETIME) AS MONTH_OF_PICKUP,
COUNT(*) AS TOTAL_PICKUP
...
------ Card
GO
CREATE VIEW PAYMENT_BY_CARD AS
SELECT 
PICKUP_DATETIME,
PASSENGER_COUNT,
TRIP_DISTANCE,
...
-- Cash
GO
CREATE VIEW PAYMENT_BY_CASH AS
SELECT
PICKUP_DATETIME,
PASSENGER_COUNT,
TRIP_DISTANCE,
